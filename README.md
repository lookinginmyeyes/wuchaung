...
如果录像集中保存在服务器磁盘，而本机只需要按需读取，可以在本机 `.env` 增加远端录像地址：

```bash
REMOTE_VIDEO_BASE_URL=http://127.0.0.1:8878
```

再打开一条 SSH 隧道，把本机 `8878` 转发到服务器平台端口：

```bash
ssh -N -L 8878:127.0.0.1:8877 ubuntu@your-server
```

这样本机后端会先查本机 `data/videos/`，找不到时再通过远端服务器流式读取录像；本机新增的实验录像也会优先上传到服务器保存，云表只保留统一的视频链接。
